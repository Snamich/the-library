...
class CollisionData implements Comparable{
    ...
    CollisionData(int f, int s, double t) {
        first = f;
        second = s;
        collisionTime = t;
        collisionId = 0;
    ...
    public String toString() {
        return " cid = " + collisionId + " " + first + " vs " + second + " @" + collisionTime + " valid = " + valid + " ";
    }
...
public class CollidingBalls extends JPanel implements ActionListener {
    ...
    double timeStep;
    int tempvar;
    CollisionTime[][] collisionTable;                // table that keeps track of time of collision between all objects.
    MinHeap collisionHeap;                      // keep track which entry is min for each row in collisionTable.
    ...
    CollidingBalls(int n) {
        ...
        collisionTable = new CollisionTime[1+nCollisionObjects] [1 + nCollisionObjects];
        collisionHeap = new MinHeap( (nCollisionObjects + 1) * (nCollisionObjects + 1) );
        for(int i=0;i<=nCollisionObjects;i++) {
            for(int j=0;j<=nCollisionObjects;j++) {
                collisionTable[i][j] = new CollisionTime( i, j, Integer.MAX_VALUE );
                collisionHeap.insert( collisionTable[i][j] );
            }
        }
    ...
    public void doCollisionWithBall(CollisionData cd){
        ...
        if (verbose) System.out.println("Handling Collisions between Ball " + balls[ballId1] + " Ball " + balls[ballId2]);
    ...
    public void computeCollisionTimes() {
        ...
        // get the time of the next collision
        double time = collisionHeap.peek().time;
        CollisionData cd = null;

        // check to see if there are additional collisions at the same time
        while( collisionHeap.peek().time <= time ) {
            CollisionTime ct  = collisionHeap.deleteMin();
            if ( ct.objectA <= nwalls) {
                cd = new CollisionData( ct.objectB, ct.objectA, time);
                int bId = ct.objectB - nwalls;
                cd.targetPos1 = Vector.addMult(balls[bId].pos, balls[bId].velocity, (time - globalTime));
            } else {
                cd = new CollisionData( ct.objectA, ct.objectB, time);
                int bId1 = ct.objectA - nwalls, bId2 = ct.objectB - nwalls;
            ...
            // readd removed item at end
            collisionTable[ct.objectA][ct.objectB].time = Integer.MAX_VALUE;
            collisionHeap.insert( collisionTable[ct.objectA][ct.objectB] );
            if(verbose) System.out.println("Adding cd =" + cd + "@ t=" + globalTime + " ccount="+collisionCount);
    ...
    public void computeCollisions(int ballId) {
        ...
        for (int i=1;i<=nballs+nwalls;i++) {
            ...
            if (i > bIndex) {
                collisionHeap.changeKey( collisionTable[bIndex][i].heapPosition, collideTime );
            }

            // Now update the column entries for which i < bIndex..
            if (i < bIndex) {
                collisionHeap.changeKey( collisionTable[i][bIndex].heapPosition, collideTime );
            }
        }
    }
    ...
    public void printTable() {
        ...
        for(int i=1;i<=nObj;i++) {
            ...
            for(int j=i+1;j<=nObj;j++) {
                if (collisionTable[i][j].time == Integer.MAX_VALUE) {
                    continue;
                } else {
                    System.out.print( "["+j+"]"+collisionTable[i][j] + ", ");
                }
            }
        }
    }
