...
init( void )
{
    ...
    // Initialize our normal and pass it to the shader
    vec4 u = vertices[0] - vertices[2];
    vec4 v = vertices[1] - vertices[2];
    vec4 normal = normalize( cross( u, v ) );

    glUniform4fv( glGetUniformLocation( program, "vNormal" ),
                  1, normal);
