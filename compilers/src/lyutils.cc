...
void token_filename(FILE *file)
{
  token_file = file;
}

void token_print(struct astree *token)
{
  fprintf(token_file, "  %2ld %3ld.%03ld %4d  %-18s (%s)\n", 
          token->filenr, token->linenr, token->offset, token->symbol, 
          get_yytname(token->symbol), token->lexinfo->c_str());
}
...
void scanner_useraction(void) {
  if (scan_echo) {
    if (scan_offset == 0) eprintf(";%5d: ", scan_linenr);
    eprintf("%s", yytext);
  }
  scan_offset += yyleng;
}
